...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-gray-200 bg-gradient-to-b from-gray-100 to-gray-50 blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(255,255,255,1)]"
      style={{ width: CELL, height: CELL }}
    />
  );
}

function Featured({ src, alt, useIcon }: { src?: string; alt?: string; useIcon?: boolean }) {
  if (useIcon) {
    return (
      <div
        className="relative shrink-0 overflow-hidden rounded-xl bg-gradient-to-br from-accent/20 to-accent/10 flex items-center justify-center"
  ...
  return (
    <div
      className="relative shrink-0 overflow-hidden rounded-xl bg-gray-200"
      style={{ width: CELL, height: CELL, boxShadow: FEATURED_SHADOW }}
    >
      <img
        src={src ?? ""}
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(220.99deg, rgba(124,197,118,0) 32%, rgb(124,197,118) 41%, rgb(173,225,147) 47%, rgba(124,197,118,0.57) 54%, rgba(124,197,118,0) 65%)",
        }}
      />
    </div>
  );
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1060px] flex-col items-stretch gap-2.5 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg outline-none focus-visible:ring-2 focus-visible:ring-accent md:min-h-[320px] md:flex-row",
        className
      )}
    >
      <div
            ...
            {middleItems.map((item, i) =>
              item.type === "featured" ? (
                <Featured
                  ...
                  useIcon={testimonials[item.i].useIcon}
                />
              ) : (
                <Cell key={i} />
              )
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          <svg
            className="block h-12 w-12 text-accent opacity-40"
            viewBox="0 0 24 24"
            fill="currentColor"
            aria-hidden="true"
          >
          ...
          >
            ...
            >
              ...
              <div>
                ...
                {current.source && <p className="text-xs text-muted-foreground mt-1">{current.source}</p>}
              </div>
            </div>
            <div
              key={displayIndex}
        ...
        <div className="mt-6 flex items-center gap-1.5 md:mt-0">
          <button
            ...
            className="grid h-6 w-6 cursor-pointer place-items-center rounded-full border border-gray-300 bg-transparent p-0 text-gray-700 transition-[opacity,transform] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:enabled:scale-[1.08] active:enabled:scale-[0.94] disabled:cursor-default disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          >
            <svg
